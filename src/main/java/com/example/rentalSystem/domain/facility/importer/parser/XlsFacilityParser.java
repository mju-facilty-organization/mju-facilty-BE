...
/**
 * .xls / .xlsx 모두 지원 (WorkbookFactory 사용)
 * 헤더는 1행 고정:
...
public class XlsFacilityParser {
    ...
    public List<FacilityRowDto> parse(MultipartFile file) {
        ...
        try (InputStream in = file.getInputStream();
            ...
            if (header == null) throw new IllegalArgumentException("헤더 행이 비어 있습니다.");
    ...
    private static Sheet firstNonEmptySheet(Workbook wb) {
        for (int i = 0; i < wb.getNumberOfSheets(); i++) {
            ...
            if (s != null && s.getLastRowNum() >= s.getFirstRowNum()) return s;
        }
        return null;
    }
    ...
    private static void ensureRequired(Map<String, Integer> colIndex) {
        ...
        for (String h : REQUIRED) {
            if (!colIndex.containsKey(h)) missing.add(h);
        }
        if (!missing.isEmpty()) {
